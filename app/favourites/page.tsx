...
const Favourites: React.FC = () => {
  ...
  return (
    <div className='flex flex-col items-center justify-center my-10 mt-40 text-white z-10 px-4 md:px-0'>
      <TextGenerateEffect 
        words='Your Favorites'
        className='text-center text-[30px] md:text-4xl lg:text-5xl text-blue-100'
      />
      {favorites.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-10 w-full max-w-6xl">
          {favorites.map(meal => (
        ...
        <p className='text-center mt-10'>No favorites added yet. Check out our exquisite menu.</p>
      )}
    </div>
  );
};
